...
試行段階では、**関連団体に所属するユーザーからのクエストのみを受注**します。
その判断材料として、クエストの審査画面に「どの団体からの申請か」が表示されます。

前提: `supabase/supabase_migration_v13_organizations.sql` を実行済みであること。

### 団体を登録する

`/admin` → **団体管理** タブ → 団体名（と説明）を入れて「追加する」。
SQL でまとめて入れることもできます:

```sql
insert into organizations (name, description, sort_order) values
  ('九州大学◯◯サークル', '', 1),
  ('△△研究室',           '', 2)
...
`/admin` → **団体管理** タブ → 団体の行の `∨` を開く → 表示名かメールアドレスで検索 → 「追加」。

> **ユーザーは自分で自分に所属を付けられません。** 付与できるのは運営だけです
> （`profile_organizations` の insert ポリシーが管理者限定）。

### ユーザーからの申請を承認する

ユーザーはプロフィール画面の「所属団体」カードから申請できます。
申請が届くと、ヘッダーの **管理** と管理画面の **所属申請** タブにバッジが出ます。

- **承認** — その場で所属が付与されます。一覧にない団体名での申請だった場合は、**承認と同時に団体が新規登録されます**
- **却下** — 理由の入力が必須です。理由は申請者のプロフィール画面に表示されます

申請メッセージは**本人と運営にしか見えません**（他のユーザーからは見えません）。

### 団体をやめるとき

**削除せず「無効にする」を使ってください。** 無効にすると新しいクエストで選べなくなりますが、
過去のクエストに残った団体名（申請時点のスナップショット）は消えません。
物理削除すると、どの団体からの依頼だったかという審査の記録が失われます。
